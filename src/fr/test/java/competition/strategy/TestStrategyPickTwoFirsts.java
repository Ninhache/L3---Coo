...
public class TestStrategyPickTwoFirsts extends TestStrategy {

	@Override
	protected IStrategyToPick initStrategy() {
		return new StrategyPickTwoFirstBest();
	}
	...
	public void testIsNotUsableForTournament() throws CompetitionIllegalCompetitorsSize {
		...
		for (int i = 0 ; i < nbGroups ; i++) {
			groups.add(new League(CompetitorFactory.createCompetitor(5), new RandomMatch()));
		}
		...
		assertFalse(this.strategy.isUsableForTournament());
	}
